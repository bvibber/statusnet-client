...
Sizzle.find = function(expr, context, isXML){
	...
	if ( !set ) {
		...
		set = Sizzle.hacks.getAllElements(context);
	}
...
var Expr = Sizzle.selectors = {
	...
	attrHandle: {
		...
		"class": function(elem){
			// For [class=foo] a raw el.class explodes interestingly on iPhone.
			// It seems to end up pulling up the raw cocoa class :D
			// Note the attrMap above is used for fetching attributes, but not
			// for the lookup comparisons.
	...
	find: {
		ID: function(match, context, isXML){
			//if ( typeof context.getElementById !== "undefined" && !isXML ) {
				var m = Sizzle.hacks.getElementById(context, match[1]);
				return m ? [m] : [];
			//}
		},
		...
		TAG: function(match, context){
			if (match[1] == '*') {
				return Sizzle.hacks.getAllElements(context);
			} else {
		...
		if ( array instanceof Array ) {
			...
			if ( typeof array.length === "number" ) {
				for ( var l = array.length; i < l; i++ ) {
					if (typeof array.item != "undefined") { // hack for nodelists on Titanium Mobile
...
(function(){
	...
	var div = Titanium.XML.parseString("<div><!-- --></div>").documentElement;
    ...
    */
	...
	div = Titanium.XML.parseString("<div><a href='#'></a></div>").documentElement;
...
(function(){
    /* -- getElementsByClassName isn't implemented, and the test fails
	//var div = document.createElement("div");

	//div.innerHTML = "<div class='test e'></div><div class='test'></div>";
	var div = Titanium.XML.parseString("<div><div class='test e'></div><div class='test'></div></div>").documentElement;
...
Sizzle.hacks = {
	getAllElements: function(context, result) {
		if (Titanium.Platform.osname == 'android') {
			...
			// getElementsByTagName('*') doesn't work on iPhone so far...
			var all = result || [];
			for (var child = context.firstChild; child; child = child.nextSibling) {
				if (child.nodeType == 1) {
					all.push(child);
					Sizzle.hacks.getAllElements(child, all); // and append its children...
				}
			}
	...
	getElementById: function(context, id) {
		if (Titanium.Platform.osname == 'android') {
			...
			// getElementById() is buggy on iPhone in 1.3.
			// We'll fake it with xpath, which on Titanium returns a DOM nodelist
			// @fixme escape or validate id string?
	...
	getElementsByName: function(context, name) {
	...
	documentElement: function(doc) {
		if (typeof (doc.documentElement) == "function") {
			// this is exposed as a function on iPhone instead of a property in 1.3
